...
to turtle-setup ;; turtle procedure
  ...
  set sugar random-in-range 0 100                                                                                       ;; NEW LINE
...
to turtle-eat ;; turtle procedure
  ...
  set sugar (sugar - metabolism + psugar) * 0.8                                                                        ;; NEW LINE first rule
  if  sugar > 80 [set sugar sugar - 5]                                                                                 ;; NEW LINE second rule
  if  sugar < 10 [set sugar sugar + 2]                                                                                 ;; NEW LINE second rule
  set psugar 0
end

to find-tax                                                                                                            ;; NEW LINE to find tax sum
  let num-company count turtles                                                                                        ;; NEW LINE
  let sorted-profit sort [sugar] of turtles                                                                            ;; NEW LINE
  let total-profit sum sorted-profit                                                                                   ;; NEW LINE
  set tax-sum-so-far (total-profit * 0.2 + count turtles with [sugar > 80] * 5 + count turtles with [sugar < 10] * -2) ;; NEW LINE
end                                                                                                                    ;; NEW LINE
...
to-report tax-income
  let num-company count turtles
  let sorted-profit sort [sugar] of turtles
  let total-profit sum sorted-profit
  set tax-sum-so-far (total-profit * 0.2 + count turtles with [sugar > 80] * 5 + count turtles with [sugar < 10] * -2)

  report tax-sum-so-far
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks tax-sum-so-far"
...
@#$#@#$#@
<experiments>
  <experiment name="population" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="tax" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>tax-sum-so-far</metric>
    <enumeratedValueSet variable="visualization">
      <value value="&quot;no-visualization&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-population">
